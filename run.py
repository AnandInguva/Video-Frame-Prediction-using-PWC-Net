...
class Run:
	def __init__(self, path1, path2):
		#self.tenFirst = torch.FloatTensor(numpy.array(path1))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		self.tenFirst = torch.FloatTensor(numpy.array(path1)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		self.tenSecond = torch.FloatTensor(numpy.array(path2)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))

		# self.tenSecond = torch.FloatTensor(numpy.array(PIL.Image.open(path2))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		self.tenOutput = estimate(self.tenFirst, self.tenSecond)
	
	def getOutput(self):
		return self.tenOutput
